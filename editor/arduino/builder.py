...
void updateTime()
{
    ...
    if (compilation.returncode == 0):
        if (port != None):
            compiler_logs += '\nUploading program to SCISONIC board at ' + port + '...\n'
            wx.CallAfter(txtCtrl.SetValue, compiler_logs)
            wx.CallAfter(scrollToEnd, txtCtrl)
            if os_platform.system() == 'Windows':
                compiler_logs += runCommand('editor\\arduino\\bin\\arduino-cli-w32 upload --port SWD' +
                                            ' --fqbn ' + platform + ' editor\\arduino\\examples\\Baremetal/')
            elif os_platform.system() == 'Darwin':
